...
struct BackendServerNode : ASTNode {
    BackendProgram program;
    std::unique_ptr<ASTNode> clone() const override {
        auto c = std::make_unique<BackendServerNode>();
        c->program = program;
        return c;
    }
};
...
public:
    ...
    static BackendProgram parseBackendFile(const std::vector<std::string>& lines) {
        return BackendParser::parse(lines);
    }
